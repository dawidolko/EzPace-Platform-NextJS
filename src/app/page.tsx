import { PLANY, KROKI, LICZBY, HISTORIE, PYTANIA } from '@/components/dane';
import { KalkulatorTempa } from '@/components/KalkulatorTempa';
...
export default function Home() {
  return (
    <>
      ...
      <section className="trail border-b border-mist-200 bg-mist-50">
        <div className="mx-auto max-w-6xl px-4 py-20 sm:px-6 lg:py-28">
          ...
          <h1 className="mt-4 max-w-3xl font-display text-5xl font-extrabold leading-[1.05] tracking-tight text-ink-900 sm:text-6xl lg:text-7xl">
            Plan cofnięty od daty startu,
            <span className="text-pine-600"> nie doklejony do niej</span>
          </h1>
          <p className="mt-6 max-w-2xl text-lg leading-relaxed text-ink-500">
            Dostajesz kalendarz z tempem dla każdej jednostki i zakresem tętna,
            a co tydzień poprawiamy go na podstawie tego, co faktycznie
            przebiegłeś. Bez gotowych szablonów z internetu.
          </p>

          <div className="mt-9 flex flex-wrap gap-3">
            <a
              href="#plany"
              className="rounded-md bg-pine-600 px-6 py-3 font-bold text-white transition hover:bg-pine-500"
            >
              Zobacz plany
            </a>
            <a
              href="#kalkulator"
              className="rounded-md border border-mist-300 bg-paper px-6 py-3 font-bold text-ink-900 transition hover:border-pine-500 hover:text-pine-600"
            >
              Policz swoje tempo
            </a>
          </div>
          ...
          <dl className="mt-14 grid gap-6 border-t border-mist-200 pt-8 sm:grid-cols-4">
            {LICZBY.map((l) => (
              <div key={l.etykieta}>
                ...
                <dd className="tabular mt-1 font-display text-3xl font-extrabold text-ink-900">
                  {l.wartosc}
                </dd>
      ...
      <section id="plany" aria-labelledby="plany-h" className="border-b border-mist-200 py-20">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          ...
          <p className="mt-3 max-w-2xl text-ink-500">
            Jeden cel, jeden termin, jedna cena. Bez abonamentu i bez
          ...
          <ul className="mt-10 grid gap-5 md:grid-cols-3">
            {PLANY.map((p) => (
              ...
              >
                ...
                <p className="tabular mt-2 font-display text-3xl font-extrabold text-pine-600">
                  {p.cena}
                </p>
                ...
                >
                  Wybieram <span className="sr-only">plan {p.nazwa}</span>
                </a>
              </li>
            ))}
          </ul>
      ...
      <section
        id="kalkulator"
        aria-labelledby="kalk-h"
        className="border-b border-mist-200 bg-mist-50 py-20"
      >
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          ...
          <p className="mt-3 max-w-2xl text-ink-500">
            Wybierz dystans i cel czasowy — dostaniesz tempo na kilometr,
            prędkość i międzyczasy, które warto zapisać na opasce.
          </p>
      ...
      <section id="efekty" aria-labelledby="efekty-h" className="border-b border-mist-200 bg-mist-50 py-20">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          ...
          <p className="mt-3 max-w-2xl text-ink-500">
            Trzy osoby, trzy różne cele. Czasy są z chipa, nie z zegarka.
          </p>
            ...
            {HISTORIE.map((h) => (
              <li key={h.imie} className="flex flex-col rounded-xl border border-mist-200 bg-paper p-6">
                ...
                <dl className="mt-4 flex items-end gap-6 border-y border-mist-200 py-4">
                  <div>
                    <dt className="text-xs uppercase tracking-wider text-ink-400">Przed</dt>
                    <dd className="tabular font-display text-xl font-bold text-ink-500">{h.przed}</dd>
                  </div>
                  <div>
                    <dt className="text-xs uppercase tracking-wider text-ink-400">Po</dt>
                    <dd className="tabular font-display text-2xl font-extrabold text-pine-600">{h.po}</dd>
                  </div>
                </dl>

                <p className="mt-4 flex-1 text-sm leading-relaxed text-ink-700">„{h.tekst}”</p>
              </li>
            ))}
          </ul>
        </div>
            ...
            {PYTANIA.map((p) => (
              <details key={p.pytanie} className="group py-4">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-4 font-semibold text-ink-900">
                  ...
                  <span
                    aria-hidden="true"
                    className="shrink-0 text-2xl leading-none text-pine-600 transition group-open:rotate-45"
                  >
                    +
                  </span>
                </summary>
      ...
      <section id="kontakt" aria-labelledby="kontakt-h" className="py-20">
        <div className="mx-auto max-w-3xl px-4 text-center sm:px-6">
          ...
          <p className="mx-auto mt-4 max-w-xl text-ink-500">
            Dwadzieścia minut o Twoim celu i kalendarzu. Bez zobowiązania —
            jeśli plan nie ma sensu w Twoim terminie, powiem to wprost.
          </p>
          <a
          ...
          <p className="mt-4 text-sm text-ink-400">
            To serwis demonstracyjny — projekt portfolio, nie działająca usługa.
          </p>
        </div>
      </section>
    </>
